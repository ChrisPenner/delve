...
ascendDir (FZ {parents=Seq.Empty, context=tree@((extract -> path -> p)), selection, ..}) = do
...
ascendDir (FZ {parents=(ps Seq.:|> (f :< pList)), context, ..}) = do
  ...
  return $ FZ {parents=ps, context=(f :< listModify (const context) pList), ..}
...
descendDir fz@(FZ {parents, context=(f:< children), ..}) = do
  ...
  return $ case listSelectedElement children of
    Nothing -> fz
    Just (_, nextChildren@(FC{kind=Dir} :< _)) -> FZ {parents=(parents Seq.|> (f:<children)), context=nextChildren, ..}
    Just _ -> fz
...
toggleSelection fz@(FZ {context=(fc:<lst), selection, ..}) = do
  ...
  return .  fromMaybe fz $ do
    ...
    let newList = listModify (const (selectedContext{selected=not isSelected} :< rest)) lst
